...
int createResponse_half(const int niterations = 10)
{
  gStyle->SetOptStat(0);
  dlutility::SetyjPadStyle();
  std::string mb_file = "TREE_MATCH_v4_8_new_ProdA_2024-00000021.root";
  std::string j10_file = "TREE_MATCH_v4_10_new_ProdA_2024-00000021.root";
  std::string j30_file = "TREE_MATCH_v4_12_new_ProdA_2024-00000021.root";
  ...
  sample_boundary_goal[2] = 30;
  sample_boundary_goal[3] = 70;

  float sample_boundary[4];
  sample_boundary[0] = 0;
  sample_boundary[1] = 0;
  sample_boundary[2] = 0;
  sample_boundary[3] = 70;
  ...
  float truth_subleading_goal = 7;

  float dphicut = TMath::Pi()/2.;
  ...
  for (int i = 0; i < nbins+1; i++)
	...
	{
	  if (sample_boundary[ib] == 0 && ipt >= sample_boundary_goal[ib]) sample_boundary[ib] = ipt;
	}
  ...
  for (int isample = 1; isample < 3; isample++)
      ...
      for (int i = 0; i < entries1; i++)
	{
	  ...
	  if (pt1_truth[isample] < sample_boundary[isample] || pt1_truth[isample] >= sample_boundary[isample+1]) continue;
	  ...
	  float e1 = pt1_truth[isample];
	  float e2 = pt2_truth[isample];
	  float es1 = pt1_reco[isample];
	  float es2 = pt2_reco[isample];
	  ...
	  bool truth_good = (e1 >= truth_leading_cut && e2 >= truth_subleading_cut && dphi_truth[isample] > dphicut);
  ...
  for (int iter = 0; iter < niterations; iter++)
    {
      h_xj_unfold[iter]->Scale(scale/h_xj_unfold[iter]->Integral());
    }
  ...
  leg->SetLineWidth(0);
  leg->AddEntry(h_xj_reco, "Reco");
  leg->AddEntry(h_xj_truth, "Truth");
  ...
  for (int iter = 0; iter < 5; iter++)
    {
      dlutility::SetLineAtt(h_closure[iter], colors[iter], 1,1);
      dlutility::SetMarkerAtt(h_closure[iter], colors[iter], 1,8);
    }
  h_closure[0]->Draw("p");
  h_closure[1]->Draw("p same");
  h_closure[2]->Draw("p same");
  h_closure[3]->Draw("p same");
  h_closure[4]->Draw("p same");
