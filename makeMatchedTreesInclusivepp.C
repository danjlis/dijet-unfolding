...
void makeMatchedTreesInclusivepp(const int sim_version = 20, const int cone_size_int = 3)
{

  std::string infile = Form("/sphenix/tg/tg01/jets/dlis/sim/pythia/Jet%d/TREE_JET_v6_%d_new_ProdA_2024-00000021.root", sim_version, sim_version);
  ...
  TEfficiency *he_pt_truth;
  TEfficiency *he_pt_dijet;
  TH2D *h2_ptt_ptrptt;
  h2_ptt_ptrptt = new TH2D("h2_ptt_ptrptt_%d","", 25, 0, 50, 120, 0, 1.2);
  he_pt_truth = new TEfficiency("he_pt_truth_%d","", 25, 0, 50);
  he_pt_dijet = new TEfficiency("he_pt_dijet_%d","", 25, 0, 50);
  ...
  for (int i = 0; i < entries; i++)
	...
	{
	  he_pt_truth->Fill(std::find_if(matched_dijets.begin(), matched_dijets.end(), [=] (auto a) {return tjet.id == a.first.id; }) != matched_dijets.end(), tjet.pt);	  
	}
      for (auto mjets : matched_dijets)
	{
	  float ptt = mjets.first.pt;
	  float ptr = mjets.second.pt;
	  h2_ptt_ptrptt->Fill(ptt, ptr/ptt);
	}
      ...
      if (mytruthjets.size() >= 2)
	{
	  ...
	  if (found_leading != matched_dijets.end() && found_subleading != matched_dijets.end())
	    {
	      he_pt_dijet->Fill(1, leading_truth_iter->pt);
	      if (Debug)
		{
	      ...
	      tn_match->Fill(max_pt_dijet_r04, pt1truth, pt2truth, dphi_truth_match, pt1reco, pt2reco, dphi_reco_match, 1, myrecojets.size(), -1, mbd_vertex_z);	      
	    }
	  else
	    {
	      he_pt_dijet->Fill(0, leading_truth_iter->pt);
	      ...
	      tn_match->Fill(max_pt_dijet_r04, pt1truth, pt2truth, dphi_truth_match, 0, 0, 0, 0, 0, -1, mbd_vertex_z);	      
	    }
	} 
    }
  he_pt_truth->Write();
